...
export const Publish = () => {
  ...
  const [newVersion, setNewVersion] = useState(
    githubData.packageJson.version ?? ""
  );
  const [commitMsg, setCommitMsg] = useState("Update Svg Asset");
  const [uploadPath, setUploadPath] = useState("packages/moeum-icons/asset");
    ...
    window.onmessage = async (event) => {
      ...
      if (type === "extractIcon") {
        if (isEmptyObj(payload?.svgByName)) {
          return setCalloutInfo({
            type: "Error",
            message: MESSAGE["EXTRACT_ICON_FAIL"],
          });
        }
        ...
        setCalloutInfo({
          type: "Success",
          message: `${itemNum} ${MESSAGE["EXTRACT_ICON_SUCCESS"]}`,
        });
  ...
  useEffect(() => {
    if (newVersion && commitMsg && uploadPath && !isEmptyObj(svgData)) {
      setDisabled(false);
    } else {
      setDisabled(true);
    }
  ...
  const handleClickPush = async () => {
    ...
    if (svgData) {
      try {
        ...
        setCalloutInfo({
          type: "Success",
          message: MESSAGE["EXTRACT_ICON_SUCCESS"],
        });
      } finally {
        setIsLoading(false);
      }
  ...
  return (
    <Box px="3">
      <Badge color="orange">
        current version : {githubData.packageJson?.version}
      </Badge>

      <Flex mt="3" justify="between" align="center">
        <Text size="1" weight="bold">
          Extract icon from selected areas
        </Text>
        <Button size="2" onClick={handleClickExtract}>
          Extract
        </Button>
      </Flex>
      ...
      <Flex mt="2" direction="column" gap="3">
        ...
        <Box>
          ...
          <TextArea
            ...
            name="commitMsg"
            value={commitMsg}
            onChange={handleOnChageTextArea}
          />
        </Box>
      </Flex>
